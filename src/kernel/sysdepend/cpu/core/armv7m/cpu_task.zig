...
pub inline fn knl_setup_context(tcb: *TCB) void {
    var ssp: *SStackFrame = *SStackFrame{undefined};
    ssp = tcb.isstack;
    ssp -= 1;
    ...
    ssp.pc = @as(*void, @as(u32, tcb.task) & ~0x00000001); // Task startup address */
    tcb.tskctxb.ssp = ssp; // System stack pointer */
}

// Set task startup code
//Called by 'tk_sta_tsk()' processing.
inline fn knl_setup_stacd(tcb: *TCB, stacd: isize) void {
    var ssp: *SStackFrame = tcb.tskctxb.ssp;

    ssp.r[0] = stacd;
    ssp.r[1] = @as(i32, tcb.exinf);
}

// Delete task contexts
inline fn knl_cleanup_context(tcb: *TCB) void {
